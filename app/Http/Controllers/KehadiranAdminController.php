...
class KehadiranAdminController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        // Gunakan session periode aktif jika tidak ada filter periode_id
        $periodeId = $request->filled('periode_id') ? $request->periode_id : session('periode_aktif_guru');
        ...
        $queryKehadiran = DetailKehadiran::with(['santri', 'jadwal.guru'])
            ...
            ->when($periodeId, fn($q) => $q->where('jadwal_mengajar.periode_id', $periodeId))
    ...
    public function getKehadiranData(Request $request)
    {
        try {
            ...
            if ($periodeId) {
                $query->where('jadwal_mengajar.periode_id', $periodeId);
            }
    ...
    public function getDokumentasiData(Request $request)
    {
        try {
            ...
            if ($request->filled('tanggal')) {
                $parseDate = Carbon::parse($request->input('tanggal'))->toDateString();
                $query->whereDate('dokumentasis.tanggal', $parseDate);
            }
